...
if(isset($_GET['service_id'])) {
  // Retrieve service ID from the URL
  $service_id = $_GET['service_id'];

  // Prepare SQL query to retrieve service information based on service ID
  ...
  if($service) {
    // Assign service data to variables for pre-filling form fields
    $serviceName = $service['service_name'];
    $serviceDescription = $service['service_description'];
    $serviceDuration = $service['service_duration'];
    $servicePrice = $service['service_price'];
  } else {
      // Handle case where service data does not exist
      // For example, redirect to a page with an error message
      header("Location: admin-service.php");
      exit();
  }
} else {
  // Handle case where service ID is not provided
  // For example, redirect to a page with an error message
  header("Location: admin-service.php");
  exit();
}
...
<div class="sidebar close">
  ...
  <section class="home-section">
    ...
    <div class="service-form-container">
        <form id="serviceForm" class="service-form" action="admin-update-service.php" method="post" enctype="multipart/form-data">
          <input type="hidden" name="service_id" value="<?php echo $service['service_id']; ?>">
          <div class="form-group">
              <label for="serviceImage">Service Image</label>
              <img src="<?php echo $service['service_image']; ?>" alt="Current Image" width="150">
              <input type="file" id="serviceImage" name="serviceImage" class="service-input" accept="image/*">
          </div>
      
          <div class="form-group">
              <label for="serviceName">Service Name</label>
              <input type="text" id="serviceName" name="serviceName" class="service-input" placeholder="Please enter the name of the service" value="<?php echo $service['service_name']; ?>" required>
          </div>

          <div class="form-group">
              <label for="serviceDescription">Service Description</label>
              <textarea id="serviceDescription" name="serviceDescription" class="service-textarea" placeholder="Please enter the description of the service" required><?php echo $service['service_description']; ?></textarea>
          </div>

          <div class="form-group">
              <label for="serviceDuration">Duration (in Hours)</label>
              <input type="text" id="serviceDuration" name="serviceDuration" class="service-input" placeholder="Enter duration" value="<?php echo $service['service_duration']; ?>" required>
          </div>

          <div class="form-group">
              <label for="servicePrice">Price</label>
              <input type="text" id="servicePrice" name="servicePrice" class="service-input" placeholder="Enter price" value="<?php echo $service['service_price']; ?>" required>
          </div>

          <div class="service-new-app-btn-container" style="width: calc(100%);">
              <!-- Form submission button -->
              <button type="submit" class="service-new-app-btn-submit btn btn-flat btn-primary"><i class='bx bx-check'></i> Update</button>
              <!-- Cancel button -->
